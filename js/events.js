...
function parseCSV(csv) {
  ...
  // Log unique categories found in the data
...
function applyFilters() {
  ...
  filteredEvents = allEvents.filter((event) => {
    ...
    // Category filter - exact match with the CSV categories
    if (category) {
      // Normalize both the event category and selected category
      const normalizedEventCategory = event.category?.trim() || "";
      const normalizedSelectedCategory = category.trim();
      ...
      if (normalizedEventCategory !== normalizedSelectedCategory) {
        return false;
      }
    }
